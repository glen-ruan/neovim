...
`空格 o` 打开浮动代码大纲后会自动进入大纲窗口。使用 `j`/`k` 选择函数、变量或类型，按 `Enter` 跳转，按 `q` 关闭；也可以输入 `/关键词` 在大纲中查找符号。浮动大纲不参与分屏布局，因此打开和关闭都不会改变现有窗口。
...
| 函数签名提示 | 插入模式按 `Ctrl+S`（Neovim 原生快捷键），圆角浮窗，最大宽度 100 列、高度 12 行；再次按可切换重载签名 |
| 关闭补全窗口 | `Esc` |
